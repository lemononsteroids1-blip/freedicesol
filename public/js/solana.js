(() => {
    ...
    const WALLET_REGISTRY = [
        ...
        {
            name: 'Solflare',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/solflare/src/icon.png',
            url: 'https://solflare.com',
            detect: () => window.solflare?.isSolflare ? window.solflare : null
        },
        {
            name: 'Backpack',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/backpack/src/icon.png',
            url: 'https://backpack.app',
            detect: () => window.backpack?.isBackpack ? window.backpack : null
        },
        {
            name: 'Glow',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/glow/src/icon.png',
            url: 'https://glow.app',
            detect: () => window.glowSolana ?? window.glow?.solana ?? null
        },
        {
            name: 'Coinbase Wallet',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/coinbase/src/icon.png',
            url: 'https://www.coinbase.com/wallet',
            detect: () => window.coinbaseSolana ?? null
        },
        {
            name: 'Trust Wallet',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/trust/src/icon.png',
            url: 'https://trustwallet.com',
            detect: () => window.trustwallet?.solana ?? null
        },
        {
            name: 'Brave Wallet',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/brave/src/icon.png',
            url: 'https://brave.com/wallet',
            detect: () => window.braveSolana ?? null
        },
        {
            name: 'Exodus',
            icon: 'https://raw.githubusercontent.com/solana-labs/wallet-adapter/master/packages/wallets/exodus/src/icon.png',
            url: 'https://exodus.com',
            detect: () => window.exodus?.solana ?? null
        }
    ];

    function _getDetected() {
    ...
    function _injectStyles() {
        ...
        s.id = 'wam-styles';
        s.textContent = `
            #wam-overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:99999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);}
            #wam-modal{background:#13161f;border:1px solid #2a2f3e;border-radius:20px;padding:0;width:380px;max-width:94vw;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.6);}
            ...
            #wam-header h2{color:#fff;font-size:17px;font-weight:700;font-family:Inter,sans-serif;}
            #wam-close{background:none;border:none;color:#8b949e;font-size:22px;cursor:pointer;line-height:1;padding:0 2px;}
            ...
            .wam-item img{width:36px;height:36px;border-radius:8px;object-fit:contain;background:#fff;padding:2px;}
            ...
            overlay.innerHTML = `
                <div id="wam-modal">
                    <div id="wam-header">
                        ...
                        <button id="wam-close">✕</button>
                    </div>
                    <div id="wam-list">
                        ${detected.map(w => `
                            <div class="wam-item" data-wallet="${w.name}">
                                <img src="${w.icon}" alt="${w.name}" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 32 32%22><rect width=%2232%22 height=%2232%22 rx=%228%22 fill=%22%23333%22/></svg>'">
                                <span class="wam-item-name">${w.name}</span>
                                <span class="wam-item-badge wam-detected">Detected</span>
                            </div>
                        `).join('')}
                        ${undetected.map(w => `
                            <div class="wam-item" data-wallet="${w.name}" data-url="${w.url}">
                                <img src="${w.icon}" alt="${w.name}" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 32 32%22><rect width=%2232%22 height=%2232%22 rx=%228%22 fill=%22%23333%22/></svg>'">
                                <span class="wam-item-name">${w.name}</span>
                                <span class="wam-item-badge wam-install">Install</span>
                            </div>
                        `).join('')}
                    </div>
    ...
    async function _sendTx(instructions) {
        ...
        const { blockhash } = await state.connection.getLatestBlockhash('finalized');
    ...
    async function settleBetOnChain(gameName, betSig, won, multiplier, amountSol) {
        ...
        try {
            const resp = await fetch('/api/settle', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ player: state.publicKey.toBase58(), game: gameName, betSig, won, multiplier, amountSol })
            });
            if (resp.ok) {
                const { settleSig } = await resp.json();
                if (settleSig) return settleSig;
            }
        } catch (_) {}
